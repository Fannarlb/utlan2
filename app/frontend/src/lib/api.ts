...
export async function addCar(licensePlate: string): Promise<Car> {
  ...
  const duplicate = cars.find(
    (c) => c.license_plate.toLowerCase() === normalized.toLowerCase()
  );
  if (duplicate) {
    throw new Error('Bíll með þessa númeraplötu er þegar skráður.');
  }

  const newId = cars.length === 0 ? 1 : Math.max(...cars.map((c) => c.id)) + 1;
  const newCar: Car = { id: newId, license_plate: normalized };
  writeToStorage(STORAGE_KEYS.cars, [...cars, newCar]);
  return newCar;
}
...
export async function removeCar(id: number): Promise<void> {
  ...
  const hasActiveLoan = loans.some(
    (loan) => loan.license_plate === car.license_plate && loan.returned === 'no'
  );
  if (hasActiveLoan) {
    throw new Error('Ekki hægt að fjarlægja bíl sem er útlánað.');
  }

  writeToStorage(STORAGE_KEYS.cars, cars.filter((c) => c.id !== id));
}
